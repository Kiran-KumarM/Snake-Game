...
export class AppComponent implements OnInit {
  ...
  updateSnake() {
    ...
    let startArray = {
      x: this.snakeBodyArray[0].x + this.currentDirection.x,
      y: this.snakeBodyArray[0].y + this.currentDirection.y,
    };
  ...
  updateFood() {
    ...
    if (
      this.snakeBodyArray.findIndex(
        (x) => x.x == foodPosition.x && x.y == foodPosition.y
      ) == -1
    ) {
  ...
  foodEaten() {
    if (
      this.snakeBodyArray[0].x == this.foodPosition.x &&
      this.snakeBodyArray[0].y == this.foodPosition.y
    ) {
      ...
      this.snakeBodyArray.push(
        this.snakeBodyArray[this.snakeBodyArray.length - 1]
      );
      this.creatingsnakeElements(
        this.snakeBodyArray[this.snakeBodyArray.length - 1]
      );
  ...
  checkIntersection(): boolean {
    if (this.snakeBodyArray.length > 3) {
      let result = this.snakeBodyArray.filter(
        (data) =>
          data.x == this.snakeBodyArray[0].x &&
          data.y == this.snakeBodyArray[0].y
      );
      if (result.length == 2) {
        return true;
      } else {
  ...
  checkOutOfBoundry() {
    if (
      this.snakeBodyArray[0].x < 1 ||
      this.snakeBodyArray[0].x > 25 ||
      this.snakeBodyArray[0].y < 1 ||
      this.snakeBodyArray[0].y > 25
    ) {
      return true;
    }
    return false;
  }
  onVirtualClick(type: any) {
    let data = { key: type };
    this.onKeypressEvent(data);
  }
